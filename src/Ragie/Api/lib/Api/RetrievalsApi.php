...
class RetrievalsApi
{
    ...
    {
        list($response) = $this->retrieveWithHttpInfo($retrieve_params, $contentType);
        return $response;
    }
     ...
     */
    public function retrieveAsync($retrieve_params, string $contentType = self::contentTypes['retrieve'][0])
    {
        return $this->retrieveAsyncWithHttpInfo($retrieve_params, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     */
    public function retrieveRequest($retrieve_params, string $contentType = self::contentTypes['retrieve'][0])
    {

        // verify the required parameter 'retrieve_params' is set
        if ($retrieve_params === null || (is_array($retrieve_params) && count($retrieve_params) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $retrieve_params when calling retrieve'
            );
        }


        $resourcePath = '/retrievals';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
        ...
        // for model (json/xml)
        if (isset($retrieve_params)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($retrieve_params));
            } else {
                $httpBody = $retrieve_params;
            }
